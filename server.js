...
// Só inicializa o Stripe se a chave existir — evita que o servidor
// caia por completo se ainda não tiveres ligado o Stripe.
const stripe = process.env.STRIPE_SECRET_KEY ? Stripe(process.env.STRIPE_SECRET_KEY) : null;

// O SDK do Cloudinary lê automaticamente a variável de ambiente
// CLOUDINARY_URL (formato: cloudinary://API_KEY:API_SECRET@CLOUD_NAME)
// se ela existir — não é preciso passar cloud_name/api_key/api_secret
// à mão. Isto evita o erro comum de copiar mal um dos 3 valores
// separadamente.
cloudinary.config();
...
// ---------------------------------------------------------------------
// Webhook do Stripe — TEM de vir antes do express.json(), porque a
// verificação de assinatura do Stripe precisa do corpo em bruto (raw),
// não já convertido para JSON.
// ---------------------------------------------------------------------
app.post('/api/stripe-webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  if (!stripe) {
    return res.status(500).send('Stripe não está configurado no servidor.');
  }
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    if (pedidoId) {
      try {
        ...
        console.log(`Pedido ${pedidoId} marcado como PAGO (confirmado pelo Stripe).`);
      } catch (err) {
        console.error('Erro ao atualizar pedido após pagamento:', err);
      }
    }
  }
...
// Mesmos ícones de categoria usados no frontend — mantidos aqui também
// para o servidor conseguir montar o HTML dos produtos de forma idêntica.
const CATEGORY_ICONS = {
  'Pulseiras': '<svg viewBox="0 0 24 24"><path d="M6 12a6 6 0 0 1 12 0 6 6 0 0 1-12 0Z"/><path d="M6 12H3M21 12h-3"/></svg>',
  'Anéis': '<svg viewBox="0 0 24 24"><circle cx="12" cy="14" r="6"/><path d="M9 8l3-5 3 5"/></svg>',
  'Brincos': '<svg viewBox="0 0 24 24"><circle cx="12" cy="7" r="2.5"/><path d="M12 9.5v6a3 3 0 0 0 3 3"/></svg>',
  'Colares': '<svg viewBox="0 0 24 24"><path d="M4 4c0 6 3.5 10 8 10s8-4 8-10"/><circle cx="12" cy="16" r="2.2"/></svg>'
};
...
// Gera o mesmo HTML dos cartões de produto que o JavaScript do cliente
// produzia — só que agora já pronto, escrito diretamente na página que
// o servidor envia, antes de qualquer JavaScript correr.
function buildProductCardsHtml(produtos) {
  return produtos.map(p => {
    const imagem = p.imagem_url
      ? `<img src="${escapeHtml(p.imagem_url)}" alt="${escapeHtml(p.nome)}" style="width:100%;height:100%;object-fit:cover;object-position:center 35%;border-radius:4px;position:relative;z-index:1;">`
      : (CATEGORY_ICONS[p.categoria] || '');
    return `
      <div class="card" data-categoria="${escapeHtml(p.categoria)}">
        <div class="swatch ${escapeHtml(p.material || '')}">${imagem}</div>
        <div class="cat">${escapeHtml(p.categoria)}</div>
        <h3>${escapeHtml(p.nome)}</h3>
        <div class="nota">${escapeHtml(p.nota || '')}</div>
        <div class="price">${fmtEuro(p.preco)}</div>
        <button class="add-btn" data-id="${escapeHtml(p.id)}">Adicionar</button>
      </div>`;
  }).join('');
}

function buildFilterChipsHtml(produtos) {
  const categorias = ['Todas', ...new Set(produtos.map(p => p.categoria))];
  return categorias.map(c =>
    `<div class="cat-chip ${c === 'Todas' ? 'active' : ''}" data-cat="${escapeHtml(c)}">${escapeHtml(c)}</div>`
  ).join('');
}

// Página inicial — agora servida pelo servidor, com os produtos já
// escritos dentro do HTML (bom para SEO e para quem partilha o link,
// já que a pré-visualização mostra conteúdo real, não uma página vazia
// à espera de JavaScript). O painel de admin e o carrinho continuam a
// funcionar exatamente como antes — nada disto muda para o cliente.
app.get('/', async (req, res) => {
  try {
    ...
    html = html.replace(
      ...
      `<div class="cat-filters" id="catFilters">${buildFilterChipsHtml(produtos)}</div>`
    );
    html = html.replace(
      '<div class="grid" id="grid"></div>',
      `<div class="grid" id="grid">${buildProductCardsHtml(produtos)}</div>`
    );
    html = html.replace(
      '<script>',
      `<script>window.__PRODUCTS__ = ${JSON.stringify(produtos)};</script>\n<script>`
    );

    res.send(html);
  } catch (err) {
    console.error('Erro ao renderizar página inicial:', err);
    // Se algo falhar, cai para o ficheiro estático normal em vez de
    // mostrar erro ao cliente — a loja continua a funcionar.
...
app.post('/api/admin/upload-imagem', exigirAdmin, upload.single('imagem'), async (req, res) => {
  ...
  if (!process.env.CLOUDINARY_URL) {
    return res.status(500).json({ error: 'Cloudinary não está configurado no servidor (falta CLOUDINARY_URL).' });
  }
...
app.post('/api/pedidos', async (req, res) => {
  ...
  try {
    ...
    // Cria a sessão de pagamento no Stripe — cartão + MB Way, com os
    // preços recalculados a partir da base de dados (nunca do frontend).
    ...
    if (stripe) {
      try {
        ...
        const stripeSession = await stripe.checkout.sessions.create({
          ...
          line_items: linhasPedido.map(item => ({
            price_data: {
              currency: 'eur',
          ...
          success_url: `${baseUrl}/sucesso.html?pedido=${id}`,
          cancel_url: `${baseUrl}/`
        });
      ...
      console.log('STRIPE_SECRET_KEY não definida — a saltar criação de checkout.');
    }

    res.json({ success: true, pedidoId: id, total, checkoutUrl });
  } catch (err) {
